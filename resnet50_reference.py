...
traindataset = torchvision.datasets.ImageNet(
        root=imagenet_path, split="train", transform=preprocess
    )
...
testdataset = torchvision.datasets.ImageNet(
...
def test(epoch, dir_path=None, plotter=None) -> None:
    ...
    if dir_path is None:
        dir_path = "outputs_base/checkpoint"
    else:
        dir_path = "outputs_base/" + dir_path
